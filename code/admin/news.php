...
$sTbl = isset($_GET['tbl']) ? $_GET['tbl'] : "news";
$sClass = $sTbl."_class";

$tpl->assign("PAGE_FUNC_BIG_LINK", "news.php");
$tpl->assign("PAGE_FUNC_BIG_NAME", "资讯管理");
...
{	
	...
	$tpl->assign("PAGE_FUNC_SMALL_NAME","添加资讯");		
	$tpl->assign("cID",$cID);		
...
elseif($sAction=="addSave") // 添加保存
{
	...
	{
		//echo "<script>location.href='news.php?act=add&cID=".$_POST['cID']."';</script>";				
		//redirect("news.php?act=listAll&cID=".$_POST['cID'],3,"添加成功！");	
		redirect("news.php?act=add&cID=".$_POST['cID']."&hangyeID=".$_POST['hangyeID'],3,"添加成功！");
	}
	else 
	{
		//redirect("news.php?act=add&cID=".$_POST['cID'],3,"添加失败！");
	}	
}
elseif($sAction == "edit" && isset($_GET['id'])) // 修改
{	
	$id = $_GET['id'];
	$tpl->assign("PAGE_FUNC_SMALL_NAME","编辑资讯");	
...
elseif ($sAction == "editSave" && isset($_POST['id'])) // 修改保存
{
	...
	if($db->update($sTbl,$aField,"id=$id"))
	{
		...
		redirect("news.php?act=listAll&status=$iStatus&cID=".$_POST['cID'],3,"修改成功！");	
	}
	else 
	{
		redirect("news.php?act=edit&id=".$id,3,"修改失败！");
	}	
}
elseif ($sAction == "listAll")  // 列表
{
	...
	$tpl->assign("PAGE_FUNC_SMALL_NAME",$sCatName."资讯列表");
	$iStatus = isset($_REQUEST['status']) ? $_REQUEST['status'] : -1; // -1 代表所有资讯
	...
	{
		$aList[$i]['catName'] = category::getCatNameByID("news_class",$aList[$i]['cID']);
		$aList[$i]['hangyeName'] = category::getCatNameByID("hangye",$aList[$i]['hangyeID']);
	}
	//print_r($aList);exit;
	dividePage("news.php", $iTotalNum, $iPerpage, $iNowPage, "act=listAll&userName=$sUserName&cID=$cID&status=$iStatus&q=$q&perPage=$iPerpage");	
...
{	
	...
	{
		redirect("news.php?act=listAll&cID=$cID&status=$iStatus",3,"成功放入回收站！");
	}	
	else
	{
		redirect("news.php?act=listAll&cID=$cID&status=$iStatus",5,"放入回收站失败！");
	}	
}
elseif($sAction == "uncheck") // 设为未审核
{
	...
	if($db->query("UPDATE $sTbl SET status='0' WHERE id in $sID"))
		redirect("news.php?act=listAll&cID=$cID&status=$iStatus",3,"成功设为未审核！");
	else
		redirect("news.php?act=listAll&cID=$cID&status=$iStatus",5,"设为未审核失败！");
}
elseif($sAction == "del") // 彻底删除
{
	...
	if($db->query("delete FROM $sTbl WHERE id in $sID"))
		redirect("news.php?act=listAll&cID=$cID&status=$iStatus",3,"删除成功！");
	else
		redirect("news.php?act=listAll&cID=$cID&status=$iStatus",5,"删除失败！");		
}
elseif($sAction == "check" || $sAction == "recommend") // 审核或推荐
{
	...
	redirect("news.php?act=listAll&cID=$cID&status=$iStatus",3,"{$sStr}成功，页面生成成功数:<font color=blue>".$iSuc."</font>；错误数：<font color=red>".$iErr."</font>！");	
}
...
elseif($sAction == "copy")//数据转移和复制
{
	...
	redirect("news.php?act=listAll&cID=$cID&status=$iStatus",3,"操作成功！");	
}
else
{
	showError("参数错误");
