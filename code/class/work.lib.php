...
class work
{
	...
	function getWorkDetail($id)
	{
		...
		$sql = "select w.id, w.name, w.cID, w.price, w.age, w.artistCode, w.picCode, w.addDate, w.comment, w.description, w.size, w.signal, w.literature, w.exhibition, w.exhibitionEName, c.name as cate_name, c.ename as cate_ename, a.name as artist_name, a.description as resume from work as w left join artist as a on a.artistCode=w.artistCode left join category as c on c.id=w.cID where w.id=".$id;
